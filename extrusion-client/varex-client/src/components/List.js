        ...
        for(let i = 1; i <= 24; i++){
            return <div>
                <div className="list-row">
                    <input type="text" placeholder="Réference matière première" value={data["ref"+i]} onChange={(e) => {
                        let newData = data
                        newData["ref"+i] = e.target.value
                        setData(newData)
                    }} />
                    <input type="number" placeholder="%" value={data["per"+i]} onChange={(e) => {
                        let newData = data
                        newData["per"+i] = e.target.value
                        setData(newData)
                    }}  />
                    <input type="text" placeholder="N° lot" value={data["lot"+i]} onChange={(e) => {
                        let newData = data
