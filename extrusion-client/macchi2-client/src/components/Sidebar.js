...
const Sidebar = () => {
    ...
    const getData = async () => {
        fetching = true;
        const fetchedData = await fetchData("/mes/navbar/macchi2", "GET");
    ...
    const checkUser = async () => {
        fetching = true;
        const fetchedData = await fetchData("/mes/checkuser/macchi2", "GET");
        if (fetchedData.success) {
    ...
    const finOF = async () => {
        const result = await fetchPHP(
            "/A_ICONS/Operator/Extrusion/macchi2/Php_Pages/fin_of.php",
            { fin_of: true }
        );
        if (result === "Success@") {
